  ...
  import unsweetend1 from '../assets/model1/m1_unsweetend.png';
  import apricot1 from '../assets/model1/m1_apricotjelly.png';
  import good_vibes1 from '../assets/model1/m1_goodvibes.png';

  import unsweetend2 from '../assets/model2/m2_unsweetend.png';
  import apricot2 from '../assets/model2/m2_apricotjelly.png';
  import good_vibes2 from '../assets/model2/m2_goodvibes.png';

  import unsweetend3 from '../assets/model3/m3_unsweetend.png';
  import apricot3 from '../assets/model3/m3_apricotjelly.png';
  import good_vibes3 from '../assets/model3/m3_goodvibes.png';
  ...
  const TryOnPopup = ({
    ...
    setSelectedModel,
    selectedLipColor = brand === 'Bobbi Brown' ? 'Downtown Plum' : 'beige',
    setSelectedLipColor,
    selectedEyeColor = 'Rose Viel',
    setSelectedEyeColor,
    selectedBlushColor = 'Unsweetend',
    setSelectedBlushColor
  }) => {
    ...
    const handleBlushColorClick = (color) => setSelectedBlushColor(color);
    ...
    const getBlushColorImage = () => {
      const blushColors = {
        model1: { 
          'Unsweetend': unsweetend1, 
          'Apricot Jelly': apricot1, 
          'Good Vibes': good_vibes1 
        },
        model2: { 
          'Unsweetend': unsweetend2, 
          'Apricot Jelly': apricot2, 
          'Good Vibes': good_vibes2 
        },
        model3: { 
          'Unsweetend': unsweetend3, 
          'Apricot Jelly': apricot3, 
          'Good Vibes': good_vibes3 
        }
      };
      return blushColors[selectedModel]?.[selectedBlushColor] || unsweetend1;
    };
    ...
    const getCurrentImage = () => {
      switch (category) {
        case 'Lips':
          return getLipColorImage();
        case 'Eyes':
          return getEyeColorImage();
        case 'Face':
          return getBlushColorImage();
        default:
          return null;
      }
    };
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div className="bg-white p-6 rounded-2xl shadow-xl max-w-2xl w-full">
          ...
          {category === 'Face' && (
            <div className="mt-4">
              <h3 className="text-lg font-medium mb-2">Select Blush Color</h3>
              <div className="flex gap-2 justify-center">
                {['Unsweetend', 'Apricot Jelly', 'Good Vibes'].map((color, index) => (
                  <button
                    key={index}
                    className={`border py-2 px-4 bg-gray-100 rounded-md hover:bg-gray-200 ${
                      color === selectedBlushColor ? 'border-pink-600 font-semibold' : ''
                    }`}
                    onClick={() => handleBlushColorClick(color)}
                  >
                    {color}
                  </button>
                ))}
